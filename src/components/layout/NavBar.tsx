...
export function NavBar({ items, className }: NavBarProps) {
  ...
  return (
    <nav
      className={cn(
        "w-full m-0 sticky top-0 left-0 right-0 z-50 py-4 lg:py-6 bg-[#0F0F0F] shadow-sm",
        className,
      )}
    >
      <div className="max-w-7xl mx-auto flex items-center justify-center px-4 sm:px-6">
        <div className="flex items-center gap-6 lg:gap-8 bg-[#0F0F0F]/95 backdrop-blur-lg py-1 px-1 rounded-full shadow-lg">
        {items.map((item) => {
