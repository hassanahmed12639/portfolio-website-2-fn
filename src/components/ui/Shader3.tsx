...
function ShaderPlane({
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      mousePos.current.x += (mouseStore.x - mousePos.current.x) * lerpFactor;
      mousePos.current.y += (mouseStore.y - mousePos.current.y) * lerpFactor;
...
const FRAGMENT_SHADER = `
  ...
  void mainImage( out vec4 fragColor, in vec2 fragCoord )
  {
    ...
    vec3 lpos = normalize(vec3(u_mouse.x * 0.5, u_mouse.y * 0.5, -1.0));
